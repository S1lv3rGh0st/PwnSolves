...
USER_TEXT_MEM = b"\x00" * MAPPING_SIZE
USER_DATA_MEM = b"\x00" * MAPPING_SIZE
USER_STACK_MEM = b"\x00" * MAPPING_SIZE
...
def handle_syscall(uc, user_data):
    log.info(f"Userland called syscall 0x{uc.reg_read(UC_X86_REG_RAX):x} and $rsi: 0x{uc.reg_read(UC_X86_REG_RSI):x}")
...
def handle_kernel_interrupt(uc, intno, data):   
    log.info("Handling kernel interrupt") 
    if intno == 0x70:
        log.info("[ ] Execution interupt!")
        rax = uc.reg_read(UC_X86_REG_RAX)
        if rax == 0:
            rdi = uc.reg_read(UC_X86_REG_RDI)
            rsi = uc.reg_read(UC_X86_REG_RSI)
            rdx = uc.reg_read(UC_X86_REG_RDX)
            log.info(f"[+] Execution mprotect: 0x{rdi:x}, {hex(rsi)} {hex(rdx)}")
            uc.mem_protect(rdi, rsi, rdx)
        elif rax == 7:
            rdi = uc.reg_read(UC_X86_REG_RDI)
            rsi = uc.reg_read(UC_X86_REG_RSI)
            rdx = uc.reg_read(UC_X86_REG_RDX)
            log.info(f"[+] Eval python code: 0x{rdi:x}, {hex(rsi)} {hex(rdx)},and text: {uc.mem_read(rdi, rdx)}")
            buf = str(eval(uc.mem_read(rdi, rdx).decode()))
            uc.mem_write(rsi, buf.encode())
...
def handle_kernel_out(uc, port, size, value, user_data):
    if port == 0x3f8 and size == 1:
        sys.stdout.buffer.write(int.to_bytes(value, 1, "little"))
